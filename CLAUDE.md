...
## NEW 기사 표시 기능
- 실행할 때마다 `last_run_keys.json`에 현재 기사 목록 저장
- 다음 실행 시 이전 목록과 비교 → 신규 기사에 초록색 **NEW** 배지 표시
- 탭 버튼에 초록 점(●), 헤더에 "✦ 이번 업데이트 신규 N건" 표시
- 첫 실행은 전체가 NEW로 표시됨 (정상)

## 주의사항
- 이혜영은 `이혜영`으로 검색하되 제목에 "이혜영" 포함된 기사만 필터링 (동명이인 방지)
